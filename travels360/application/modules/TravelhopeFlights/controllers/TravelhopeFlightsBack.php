...
class TravelhopeFlightsBack extends MX_Controller
{
    ...
    public function update_settings()
    {
        ...
        $this->App->service("ModuleService")->update('TravelhopeFlights', 'apiConfig', $payload['apiConfig']);
        redirect(base_url('admin/thflight/settings'));
	}
    ...
    public function bookings()
    {
        ...
        $xcrud->table('travelhope_flights_bookings');
        $xcrud->columns('id, session_key, booking_id, origin, destination, departure_date, arrival_date, adults, children, infants, created_at');
        $xcrud->order_by('id', 'desc');

        $xcrud->button(base_url('thflights/invoice/{id}'), 'View Invoice', 'fa fa-search-plus', 'btn btn-primary', array('target' => '_blank'));
